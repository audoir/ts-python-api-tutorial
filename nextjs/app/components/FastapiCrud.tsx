...
export default function FastapiCrud() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (editingItem) {
        const res = await fetch(`${FASTAPI_API}/api/items/${editingItem.id}`, {
          method: "PUT",
  ...
  async function handleDelete(id: number) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${FASTAPI_API}/api/items/${id}`, {
        method: "DELETE",
      });
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="border border-gray-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-gray-800 dark:text-zinc-100 bg-white dark:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-teal-400"
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          <label
            className="text-sm font-medium text-gray-600 dark:text-zinc-400"
            htmlFor="fastapi-description"
          >
            Description
          </label>
          <input
            id="fastapi-description"
            type="text"
            value={description}
        ...
        <div className="flex gap-3">
          <button
            type="submit"
            disabled={loading}
            className="bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-medium rounded-lg px-5 py-2 transition-colors"
          >
